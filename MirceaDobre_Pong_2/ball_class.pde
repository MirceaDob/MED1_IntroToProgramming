...
void wallCollision() { //collision between the ball and the walls
 if (Start) {
  ...
  void scoreBall() {
  if ((balX > width - balH / 2 - pad1W) && (balY > mouseY - pad1H / 2) && (balY < mouseY + pad1H / 2)) { //if the ball hits the pad it bounces off and the score increases
      spdX = spdX * 1.1;
      spdX = -spdX;
      balX = balX + spdX;
      scor = scor + 1;
  }
  if (balX > width) { //if the ball hits the right wall instead of the pad the score decreases and the ball starts from the initial position
    Start = false;
    balX = random(width);
    balY = random(height); 
    spdX = random(3, 5);
